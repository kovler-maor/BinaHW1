...
class OnePieceProblem(search.Problem):
    ...
    def h(self, node):
        ...
        number_of_pirate_ships = len(state["pirate_ships"])
        alpha = number_of_pirate_ships / number_of_treasures
        h_1 = self.h_1(node)
        h_distance_to_treasures = self.h_distance_to_treasures_that_are_not_on_ship(state)
        h_distance_to_base = self.h_distance_to_base(state)
        return h_1 * alpha + h_distance_to_treasures * 1 + h_distance_to_base * 3
    ...
    def h_distance_to_treasures_that_are_not_on_ship(self, state):
        ...
        for treasure in treasures:
            # check which cells are adjacent to the treasure and in the map
            treasure_position = treasures[treasure]["position"]
            adjacent_cells = [(treasure_position[0] + 1, treasure_position[1]),
                              (treasure_position[0] - 1, treasure_position[1]),
                              (treasure_position[0], treasure_position[1] + 1),
                              (treasure_position[0], treasure_position[1] - 1)]
            ...
            if not treasures[treasure]["is_on_ship"] and not treasures[treasure]["is_collected"]:
                for pirate_ship in pirate_ships:
                    if is_ship_full(pirate_ships[pirate_ship]):
                        continue
                    min_distance = math.inf
                    for cell in adjacent_cells:
                        if state["map"][cell[0]][cell[1]] != "I":
                            distance = L1_distance(pirate_ships[pirate_ship]["position"], cell)
                            if distance < min_distance:
                                min_distance = distance
                    sum_of_distances += min_distance
            average_distance = sum_of_distances / (len(pirate_ships) * len(treasures))

        return average_distance

    def h_average_distance_to_treasures(self, state):
        pirate_ships = state["pirate_ships"]
        treasures = state["treasures"]
        sum_of_distances = 0
        average_distance = 0
        for pirate_ship in pirate_ships:
            for treasure in treasures:
                ...
                distance = L1_distance(pirate_ships[pirate_ship]["position"], treasures[treasure]["position"])
                sum_of_distances += distance
            average_distance += sum_of_distances / len(treasures)
        return average_distance / len(pirate_ships)
    ...
    def h_distance_to_base(self, state):
        ...
        for treasure in treasures:
            adjacent_sea_cells = get_adjacent_sea_cells(state, treasures[treasure]["position"])
            ...
            if not is_collected:
                if not is_departed:
                    ...
                    for cell in adjacent_sea_cells:
                        if state["map"][cell[0]][cell[1]] != "I":
                            distance = L1_distance(base_position, cell)
                            if distance < min_distance:
                                min_distance = distance
                    sum_of_distances += min_distance
        ...
        return sum_of_distances/len(treasures)
...
def get_adjacent_sea_cells(state, position):
    ...
    return [cell for cell in adjacent_cells if 0 <= cell[0] < len(state["map"]) and
            0 <= cell[1] < len(state["map"][0]) and state["map"][cell[0]][cell[1]] != "I"]
